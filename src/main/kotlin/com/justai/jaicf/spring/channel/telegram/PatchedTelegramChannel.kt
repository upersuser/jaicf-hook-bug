package com.justai.jaicf.spring.channel.telegram
...
class PatchedTelegramChannel(
    ...
    private val tgRequestExecutor = ThreadPoolTaskExecutor().apply {
        corePoolSize = 10 //100
        maxPoolSize = 20 //200
        setAwaitTerminationSeconds(1) //было 15, дефолт 10
        setWaitForTasksToCompleteOnShutdown(true)
        threadNamePrefix = "tg-request-executor-"
        initialize()
    }
    ...
    val bot = bot {
        ...
        dispatch {
            fun process(request: TelegramBotRequest) {
                tgRequestExecutor.execute {
                    botApi.process(request,
    ...
    companion object : JaicpCompatibleAsyncChannelFactory {
        ...
        ) = PatchedTelegramChannel(botApi, telegramApiUrl = apiUrl, telegramBotToken = "").apply {
